...
2. Resolve this workspace's project key from `git remote get-url origin`: drop scheme, credentials
   and host, strip a trailing `.git`. At onboarding this resolved to `DevilDogTG/ETS2Mods.BetterFlaresHeadlightVisibilityAddOns`. If `origin`
   has changed since, `origin` wins — re-run the onboard script.
3. Read the workspace directives from `~\.agent-brains\projects\DevilDogTG/ETS2Mods.BetterFlaresHeadlightVisibilityAddOns\AGENT.md`.
4. Use `~\.agent-brains\projects\DevilDogTG/ETS2Mods.BetterFlaresHeadlightVisibilityAddOns\memory\` for project context.
5. Always write plans to `~\.agent-brains\projects\DevilDogTG/ETS2Mods.BetterFlaresHeadlightVisibilityAddOns\plan\` BEFORE writing code.
...
- [SK] workspace:<id>       -> `~\.agent-brains\projects\DevilDogTG/ETS2Mods.BetterFlaresHeadlightVisibilityAddOns\skills\<id>\<id>.md`
